...
class PLC(object):
    ...
    def SetPLCTime(self, dst=None, set_timezone=False, timezone=None):
        """
        Sets the controller clock time to match the PC running pylogix.

        dst:          daylight saving flag.  Defaults to the PC's current
                      DST state when not provided.
        set_timezone: when True, also set the controller's time zone to
                      match the PC.  The zone is written as a plain
                      "UTC+/-HH:MM" string built from the PC's standard
                      (non-DST) offset.
        timezone:     optional explicit time zone string to write, e.g.
                      "UTC-05:00 Eastern Time (US & Canada)".  Providing
                      this implies set_timezone=True.
